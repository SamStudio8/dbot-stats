    ...
    var commands = {
            ...
            if(event.params[1]){
                ...
                if(dbot.db.userStats.hasOwnProperty(input)){
                    event.reply(dbot.t("user_lines", {
                        "user": input,
                        "chan": event.channel,
                        "lines": dbot.db.userStats[input][event.channel]["total_lines"].numberFormat(0),
                ...
                event.reply(dbot.t("chan_lines", {
                    "chan": event.channel,
                    "lines": dbot.db.chanStats[event.channel]["total_lines"].numberFormat(0),
        ...
        '~lincent': function(event){
            if(event.params[1]){
                ...
                var percent = ((dbot.db.chanStats[event.channel]["users"][event.user]["lines"]
                    / dbot.db.chanStats[event.channel]["total_lines"])*100);
                event.reply(dbot.t("lines_percent", {
                    "user": event.user,
                    "chan": event.channel,
        ...
        '~loudest': function(event){
            ...
            for(var user in dbot.db.chanStats[event.channel]["users"]){
                if(dbot.db.chanStats[event.channel]["users"].hasOwnProperty(user)){
                    if(dbot.db.chanStats[event.channel]["users"][user]["lines"] > max){
                        max = dbot.db.chanStats[event.channel]["users"][user]["lines"];
                        max_user = user;
                    }
                }
            }
